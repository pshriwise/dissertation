...
\subsection{Application in Embree}
...
ray-box tests in Embree contain no branching (if-else statements). This avoids
any additional memory fetching or re-organizing of CPU registers so that many
SIMD operations can efficiently performed in sequence. This is accomplished by
some precomuptation of ray values at the beginning of the BVH traversal such as
the inverse ray direction, SIMD computation of ray intersection locations for
each coordinate of the box, and bulk comparison of intersection distances using
SIMD instructions.
...
An additional optmization is the precomputation of
the near and far coordinates of bounding boxes. This is used to precompute which
of the axis-aligned box coordinates the ray will strike first and last. 
