...
\geomQuery{Point Containment}{plc_query.eps}{0.5}{
...
\geomQuery{Next Surface}{dtb_query.eps}{0.3}{
...
\geomQuery{Closest Surface}{ctl_query.eps}{0.3}{
...
\geomQuery{Next Volume}{next_vol_query.eps}{0.5}{
...
\geomQuery{Surface Normal}{normal_query.eps}{0.3}{
...
\geomQuery{Measure}{measure_query.eps}{0.3}{
...
\begin{lstlisting}[language=Python,basicstyle=\tiny,caption={General algorithm for spatial hierarchy traversal to return the nearest intersection along a ray.},label={alg:bvh_traversal},captionpos=b]
...
heuristic is to maintain the expected $O(log(N_{triangles})$ cost of a ray
...
\ref{eq:ERH}. The ERH cost is unitless and bounded by zero and one. This
...
\end{equation}
  ...
  \end{align*}
  \caption{An example of the ERH calculation for a binary tree.}
  \label{fig:ERH}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.3]{2d_kd_eg.eps}
...
\footnotetext{Some notation has been modified to agree with
  notation used earlier in in Figure \ref{fig:SAH}}
...
\begin{figure}[H]
  ...
  \centering
  \caption{2D visualization of a signed distance field surrounding an arbitrary volume boundary.}
