...
geometric vertices. For the purposes of this work, only the relationship between
volumes and surfaces are of concern. A depiction of the relationship between
these entities in the mesh database can be seen in Figure \ref{fig:dagmc_geom_example}.

\begin{sidewaysfigure}
  \centering
  \includesvg{../images/moab_geom_fig}{0.45\textwidth}
  \includesvg{../images/mesh_based_topology}{0.5\textwidth}
  \caption{Left: Partial representation of a discretized DAGMC geometry. Right:
...
\textit{Next Surface} and \textit{Closest Surface} (see Figures
\ref{fig:Next Surface} and \ref{fig:Closest Surface}) geometry queries, for
...
Equation \ref{eq:ERH}. The ERH cost is unitless and bounded by zero and
...
known as voxels, are used to partition the 3D problem space into the eight octants
defined by the global axes and extents of the parent voxel. These eight voxels are
...
\begin{figure}[H]
  ...
  \centering
  \caption{2D visualization of the signed distance field.}
...
This discussion of the CAD-based transport workflow and methods in DAGMC,
hierarchical spatial data structures, and generation of SDFs in this chapter
provides a basis for the work performed in the remainder of this document. The
following chapter, Chapter \ref{ch:preconditioning} discusses the implementation
and application of SDFs in DAGMC's framework. Next, Chapter \ref{ch:simd_bvh}
presents work on BVHs optimized for use in DAGMC, and extensions
of those methods to provide robust transport in production DAGMC
problems. Chapter \ref{ch:high_valence} discusses the effects of splitting
heuristics and hierarchy construction surrounding particular geometric features
on the performance of DAGMC simulations.
