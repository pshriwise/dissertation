...
compilers and code writers to initiate requests for memory access before it's
needed. This technique, known as memory prefetching, avoids memory latency by
populating low level memory caches with data before it is required by the
CPU. 

\begin{figure}[H]
\centering
\includesvg{../images/mem_latency.png}{width=0.9\textwidth}
...
including next-line and stride prefetching. These patterns are
often applied when looping over large arrays of data. Stride prefetching is used
when many elements of an array are operated at the same time and the next stride
of elements is needed in the next iteration. Examples of both next-line and stride
prefetching can be seen in \ref{fig:prefetching examples}. Modern copmilers are
becoming increasingly adept at predicting memory access for these cases and will
apply prefetching as part of resulting instruction sets.
...
user-informed prefetching method is pointer-based prefetching. In this method,
...
\begin{figure}[H]
\centering
\includesvg{../images/prefetch-loop}{width=0.8\textwidth}
\caption[Manual memory prefetching example.]{Example of manually implemented
  memory prefetching for a standard loop performing a vector operation.}
...
Embree manually prefetches node information 
During traversal of Embree's BVH, nodes at the top of the traversal stack are
...
\paragraph{Vectorized Ray-Box Intersection Tests}
