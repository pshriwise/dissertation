...
surface area, the average chord length ($R_{av}$). (INSERT_WEIGNER_REFERENCE_HERE) This value can be defined as
seen in Eq. \ref{eq:average_chord_length}.

\begin{equation}
\centering
 R_{av} = \frac{4V}{S}
 \caption{Definition of the average chord length of a volume where $V$ is the volume and $S$ is
          the surface area. }
...
average chord length is a way of describing the likelyhood that a particle will
...
found in Figures INSERT_REFERENCE_TO_FIGURES HERE. The results of these tests
...
breaks down for values of n larger than
