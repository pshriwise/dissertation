...
contribution to the simulation solution. Occasional \textit{point\_containment}
...
by the \textit{make\_watertight} algorithm used to topologically seal DAGMC
...
\begin{table}[H]
  ...
  \caption[SDF production simulation times using DAGMC's physics distance
    optimization.]{Results of performance improvements, result matching, and
    lost particle incidence in the SDF production models when using DAGMC's
    physics distance limit mode.}
...
It is worth noting that the errant particle pathology associated with the
...
to cross surfaces due to the error associated with the interpolation of these
values so long as the model is well-formed. Rays fired in simulations where SDF
preconditioning is applied still have an effectively infinite length and thus do
not invalidate the robust particle tracking process in DAGMC. Particles moving
through gaps in unsealed volumes will always be reported by DAGMC. 

The SDF also has the potential to avoid lost particles in some scenarios. In its
standard operation, DAGMC requires that a surface intersection be found for each
infinite-length ray it fires. 
