...
\begin{figure}[H]
  \begin{center}
    ...
    \caption{CAD representations of the sphere, slotted sphere, and high aspect
      ratio cylinder (left to right) test models used for ray fire timings of DAGMC and
      EmDAGMC. \label{models}}
  \end{center}
\end{figure} 
...
\begin{sidewaysfigure}[]
  ...
  \caption{Ray fire test results on three representative DAGMC volumes shown in Figure \ref{models}. Each
    ...
    two cases due its lower performance.}
...
\begin{table}[H]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Runtime comparison native MCNP, DAG-MCNP, and EmDAG-MCNP over four contrived
...
\begin{figure}
  ...
  \caption{Call-graph of the EmDAG run on the FNG model for \num{1E7}
...
model. Imprinting (shown in in Figure \ref{fig:imprit_ex} is the process by which
...
\begin{figure}[h!]
  \begin{centering}
    ...
    \caption{\textbf{A)} The initial state of the lost particle. The
      ...
      placement of the particle in volume B the Monte Carlo code requests
...
Equation \ref{eq:box_ext_max} indicates that boxes will need to be expanded an amount $\Delta$
...
%  \includesvg{../images/box_extension_test}{width=0.8\textwidth}
  \caption{A study of ray tracing performance and robustness for various values of
    the box extension parameter, $\Delta$. For varying faceting tolerances,
    600,000 rays were fired at each model. Points in the graph represent the
    average over all faceting tolerances used.}
...
can be evaluated as seen in Equation \ref{eq:simplified_sah}.
...
\begin{sidewaysfigure}
  ...
  \caption{Left: Representation of a faceted DAGMC geometry. Right: The
    corresponding BVH structure for that volume. Surface sub-trees are shared
...
combined to denote the number of primitives in that leaf node. This places a
...
\begin{figure}[H]
  ...
  \caption{Ray structures used to communicate hit information between the MPBVH
...
\begin{table}[H]
  ...
  \caption{Runtime comparison native MCNP, DAG-MCNP, and DAG-MCNP using the
    MPBVH over four transport test problems. No lost particles occurred in any
    of these runs and all results (see Appendix \ref{ch:appendix-c}) match the
    standard DAGMC implementation exactly.}
...
production models as well. No particles were lost in any of these geometries,
and the results are unchanged between versions of DAGMC.
...
\begin{table}
  ...
  \begin{center}
    \begin{tabular}{lc}
      ...
      \toprule
      \textbf{Test Model} & \textbf{\thead{Memory Usage Ratio \\ to MOAB OBB Tree}} \\
      \hline
      FNG           & 0.83 \\
      ATR           & 0.80 \\
      UWNR          & 0.77 \\
    ...
    \end{tabular}
  \end{center}
  \caption{Memory usage of the BVH structures using both MOAB's OBB
    tree and the MPBVH.}
