...
\subsubsection{EmDAG Performance Testing}\label{subsubsec:emdag_rf_performance}
...
\begin{figure}[H]
  ...
  \begin{minipage}{.5\textwidth}
    \centering
    \includegraphics[scale=0.33]{Eig_fix_rf.eps}
  \end{minipage}%
  \begin{minipage}{.5\textwidth}
    \centering
    \includegraphics[scale=0.33]{embree_rf.eps}
  \end{minipage}
...
the MPBVH and MOAB OBBTree are consistent. Several unit tests in the kernel fire
...
performance and in partricle transport, though none were found outside of the
controlled box extension study in Section \ref{subsubsec:reduced_precision}.

\section{Ray Fire Performance}\label{sec:mpbvh_rf_perf}
...
\subsection{Simplified Particle Tracking}
...
This algotrithm avoids many logical checks and the classification of hit
...
algorithm by B. Smight are as follows \cite{Smith_2011}:
...
intersection test and extending ray boxes to ensure the corect triangle is
visited for intersection. Part \textbf{b)} is addressed by applying a small
tnear value to the ray when firing in the negative direction and limiting the
...
eixiting than entering hits are found, then the particle is considered to be
inside the volume. Otherwise it is considered to be outside of the volume.

This algorithm was applied to all of the simluation results presented in this
chapter without lost particles or causing infinite loops.

\section{Simulation Results}

\subsection{Simple Test Cases}\label{subsec:mpbvh_simple_tests}
...
\begin{table}[H]
  ...
  \caption{Runtime comparison native MCNP, DAG-MCNP, and DAG-MCNP using the
    MPBVH over four transport test problems. No lost particles occured in any of
  these runs and all results match the standard DAGMC implementation exactly.}
  \label{tab:mpbvh_transport_timing_simple}
\end{table}

\subsection{Production Simulations}

After verifying that no particles were lost in the simple test cases, the MPBBVH
implementation of DAGMC was applied to the same FNG model as EmDAG. The results
of this simluation can be seen in Table
...
tallly results matched the standard DAGMC implementation with reduction in the
