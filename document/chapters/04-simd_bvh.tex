...
The node reference type is identified by toggling the least significant bit in
...
on the returned ray. The depth-first traversal of the hierarchy protects against
...
This node type also provides the ability to avoid the duplication of triangle
...
best tested using MOAB mesh models. These robustness tests fire rays from the
center of several single-volume MOAB/DAGMC models with rays directed at the
...
\subsection{Plucker Ray-Triangle Intersections}\label{sec:intersection_distances}
...
\section{Particle Transport Testing}
