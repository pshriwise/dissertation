...
each of these tests, the models are tessellated with an increasingly smaller
...
\begin{itemize}
  ...
  \item templating of vector types to support SIMD instruction sets
  \item memory pre-fecthing of node memory added to the traversal stack
  \item precomputation of near and far box boundaries
...
connectivity. Access to these locations in memory is handled by an object called
the MOABDirectAccessManager which protects the mesh information from
modification during the building and traversal of the BVH.
