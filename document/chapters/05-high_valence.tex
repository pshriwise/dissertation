...
\begin{figure}[H]
  ...
  \caption{Examples of HV vertices in production models.}
...
\begin{figure}[H]
  ...
  \begin{center}
    ...
    \caption{Results of MOAB ray tracing performance tests with decreasing
      ...
      represent average time spent in firing a ray for random rays originating
      at the center of each model\cite{Tautges_2009}. The ``deathstar'' model is
      the same as the slotted sphere model used in tests performed by the
      author.}
...
\begin{sidewaysfigure}
  ...
  \caption{HV characterization study results for all MOAB OBB tree
    implementations. Top Left: Unmodified MOAB results. Top Right: Manual
    modification of MOAB OBB tree's build settings. Bottom: Results with
    adaptive construction for HV regions.}
...
queries on this model. Information on this test program can be found in Appendix
\ref{ch:appendix-c} of this work. This program was used to fire rays with random
...
\begin{figure}[H]
  \centering
    ...
    \caption{View of culprit bounding box containing many HV region
      triangles as well as other surface triangles. Blue box and triangles: leaf
      ...
      representative OBBs at the same depth in that tree.}
...
\begin{table}[H]
  ...
  \begin{tabular}{c c c c c}
    \toprule
    \textbf{\thead{Model}} & \textbf{\thead{HV \\ regions}} & \textbf{\thead{Particle \\ histories}} & \textbf{\thead{Run Time \\ reduction}} & \textbf{\thead{Buildtime \\ increase}} \\
    \hline
    FNG            & 514                 & 1M                          & 3.3\%                      & 18.5\%                      \\
    ITER           & 3522                & 1M                          & 23.4\%                     & 5.2\%                       \\
    \bottomrule
  \end{tabular}
  \caption{Results of run time reduction in several DAGMC production models when applying the BVH refinement.}
...
in direct proportion to both valence and the relative area of the HV
region. This data presents the behavior originally expected of MOAB's BVH. Unlike MOAB
however, Embree's use of the surface area heuristic prevents the artificial
increase in probability of visiting triangles in the HV region. Larger
...
\begin{figure}
  ...
  \caption{MPBVH Axis-Aligned leaf node for the HV test model with area fraction
    0.5 and a valence of 100.}
...
\begin{figure}[H]
  ...
  \caption{Ray fire tests on three representative DAGMC volumes. Each data point
...
\begin{figure}[H]
  ...
  \caption{Variation in run time for different HV detection parameters on a
