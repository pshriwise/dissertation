  ...
  \begin{table}[H]
    ...
    \begin{center}
      \begin{tabular}{lcc}
        ...
        Average & 7.222176E+12 & \DIFdelbeginFL \DIFdelFL{7.217213}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{7.222176}\DIFaddendFL E+12     \\
        Maximum & 1.410510E+14 & \DIFdelbeginFL \DIFdelFL{1.410040}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.410510}\DIFaddendFL E+14     \\
...
%DIFDELCMD <     \centering
%DIFDELCMD <     \includesvg{../images/iter_mpbvh_meshtally_cdf}{width=\textwidth}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption[Analysis of differing mesh tallies in ITER.]{%
{%DIFAUXCMD
\DIFdelFL{A cumulative
      distribution function for the differing tally values between DAG-MCNP and
...
\DIFdelend %DIF > % \begin{figure}[H]
  %DIF > %   \centering
  %DIF > %   \includesvg{../images/iter_mpbvh_meshtally_cdf}{width=\textwidth}
  %DIF > %   \caption[Analysis of differing mesh tallies in ITER.]{A cumulative
  %DIF > %     distribution function for the differing tally values between DAG-MCNP and
  %DIF > %     DAG-MCNP using the MPBVH for the ITER model. Zero entries for the
  %DIF > %     unmodified DAG-MCNP simulation are set to zero here. }
  %DIF > % \end{figure}
...
%DIFDELCMD <         %%%
\DIFdelFL{Number of Tally Values }%DIFDELCMD < & %%%
\DIFdelFL{Standard Deviation }%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdelFL{0.50 }%DIFDELCMD < & %%%
\DIFdelFL{59.69 }%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdelFL{0.75 }%DIFDELCMD < & %%%
\DIFdelFL{108.77 }%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdelFL{0.90 }%DIFDELCMD < & %%%
\DIFdelFL{162.93 }%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdelFL{0.95 }%DIFDELCMD < & %%%
\DIFdelFL{213.71 }%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdelFL{0.99 }%DIFDELCMD < & %%%
\DIFdelFL{353.51 }%DIFDELCMD < \\
...
\DIFdelend %DIF > % \begin{table}[H]
  %DIF > %   \begin{center}
  %DIF > %     \begin{tabular}{cc}
  %DIF > %       Number of Tally Values & Standard Deviation \\
  %DIF > %       0.50 & 59.69 \\
  %DIF > %       0.75 & 108.77 \\
  %DIF > %       0.90 & 162.93 \\
  %DIF > %       0.95 & 213.71 \\
